...
export class Session {
  ...
  endAttempt(): void {
    throw new Error('Task 10에서 구현한다');
  }
...
`endAttempt`가 `throw`하던 자리부터 `finishGame`까지를 통째로 아래로 바꾼다:
...
- Produces: `class CircleCanvas` — `constructor(el, opts)`, `render(strokes)`, `onStroke(fn)`, `clear()`
...
export class CircleCanvas {
  ...
  private resize(): void {
    ...
    const size = Math.max(1, Math.min(this.el.clientWidth, this.el.clientHeight));
    this.el.width = Math.round(size * dpr);
    this.el.height = Math.round(size * dpr);
    this.draw();
  }
...
- [ ] **Step 5: 결과 연출을 임시로 비워둔다**

Task 14에서 채운다. 지금은 원본만 그린다.
...
/** Task 14에서 조립 연출로 바꾼다. 지금은 원본만 그린다. */
export function revealRound(
  el: HTMLCanvasElement,
  drawing: Point[][],
  _sliceCount: number,
  _owners: Array<{ sliceIndex: number; playerId: string | null }>,
  _youId: string,
): void {
  const ctx = el.getContext('2d')!;
  const dpr = window.devicePixelRatio || 1;
  ...
  ctx.save();
  ctx.scale(el.width / CANVAS, el.width / CANVAS);
...
**"아하" 순간이자 이 게임의 클립 소스.** 흩어진 조각이 회전이 풀리며 제자리로 날아가 붙는다.

**Files:**
- Modify: `src/client/reveal.ts`

**Interfaces:**
- Consumes: `slice` (Task 4), `CANVAS`/`CENTER`/`RADIUS` (Task 2)
