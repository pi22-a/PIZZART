...
| | 어디서 | 포트 | 무엇을 |
|---|---|---|---|
| **공개** | **AWS Lightsail** (태그에 고정) | 8090 | `pizzagame.app` — 24시간 |
...
이 한 줄이 SSH → pull → 빌드 → 재시작 → 확인까지 한다. 손으로 하면 다섯 단계라
빠뜨리기 쉬웠다.

**서버는 `main`을 따라간다.** 태그에 고정해 두던 방식은 매번 SSH로 들어가 checkout을
해야 해서 번거로웠다. 대신 **배포 전에 태그를 찍는 규율은 그대로다** — 스크립트가 태그
없는 main은 거절한다. 태그가 없으면 "지금 뭐가 떠 있나"에 답할 수 없고 되돌릴 지점도
안 남는다.
...
| 확인 | 왜 |
|---|---|
| 작업 트리가 깨끗한가 | 안 올린 변경을 올렸다고 착각하지 않게 |
| main == origin/main | 로컬에만 있는 커밋을 올린 줄 알지 않게 |
| main에 태그가 있는가 | 되돌릴 지점을 남기려고 |
| **지금 방에 사람이 있는가** | **재시작하면 그 판이 리셋된다** |

마지막이 가장 중요하다. 방 목록을 받아 사람이 있으면 멈추고, 어느 방에 몇 명이
무엇을 하는 중인지 보여준다. 정말 지금 해야 하면 `--force`가 있다.

```
$ npm run deploy
  올릴 것: v1.8.0
  지금 2개 방에 사람이 있습니다:
    · 초코비의 방 — 5명, 게임 중 (라운드 3/5)
    · 새 방 — 4명, 대기 중
  올리면 이 판들이 리셋됩니다.
```
...
방 상태가 **서버 메모리에만** 있어서다. 어떤 방식으로 올리든 새 코드가 돌려면 프로세스를
다시 띄워야 하고, 그 순간 방이 사라진다. 브랜치를 바꾸든 pull을 하든 마찬가지다.

다만 **끊김 자체는 가볍다.** 실측으로 재시작은 1초쯤이고, 클라이언트에 자동 재접속이
있어서 사람들은 같은 방으로 알아서 돌아온다 — 돌던 판이 리셋될 뿐 튕겨 나가 못 돌아오는
것이 아니다.

진짜 무중단을 원하면 **방을 디스크에 저장했다 되살려야 한다.** 작지 않은 작업이라
"사람 없을 때 올린다"로 미뤄두고 있다. 규모가 커져 상시 접속자가 생기면 그때 넣는다.
